...
JetBrains keynote recap: https://blog.jetbrains.com/kotlin/2026/05/kotlinconf26-keynote-highlights/ ("Advances in Language Design, Tooling, AI-Driven Workflows, and Multiplatform Development").
...
- Still experimental in 2.4: auto-generated `equals`/`hashCode`/`toString` for value classes, collection-literal updates, locality as a first-class language concept, "rich errors" for recoverable failures.
...
- **DataStore on Web**: reactivity comes via `StorageEvent`; OPFS lacks `StorageEvent`, solved with a custom solution.
...
> Zac Sweers. "Metro is both a multiplatform DI framework and a sophisticated Kotlin compiler plugin. This advanced session breaks down how Metro works inside the compiler, what code it generates, and how its 'magic' actually happens."
...
> Sam Berlin and Alessio Della Motta (Google, Search Infra). "Discover how Google Search uses server-side Kotlin and coroutines to enable low-latency, highly asynchronous streaming code paths at massive scale." Introduces Qflow, a data-graph interface language connecting async definitions with Kotlin business logic, plus coroutine instrumentation for latency tracking and critical-path analysis.
...
> Presented by the JetBrains team. Per Touchlab: "goes over the history of KotlinJs while also talking about what is next."
...
> Per Touchlab: covers **App Platform**, a library by Amazon for state management, dependency injection, and decoupling logic layers; includes a deep dive into iOS integration challenges and architectural decisions. Library currently at v0.0.8.
...
> Jake Wharton (Skylight). "Modern terminals can do far more than print text. In this deep dive, Jake explores how command-line apps communicate with terminals - from colors and sizing to advanced features like frame sync, images, and keyboard events. Using Kotlin, he covers OS-specific APIs, JVM vs. Kotlin/Native challenges, and reusable libraries that help you unlock the full power of the terminal."

- Communication is bi-directional; stdin / stdout / stderr flow between the terminal and the executable.
- ANSI control sequences drive the output.
...
> Tadeas Kriz (Touchlab). "Kotlin Multiplatform native builds come with a key constraint: one native binary per project. This session explores what happens when multiple binaries enter the picture, the architectural impact on large systems, and strategies for splitting compilation into manageable parts. It's a practical look at scaling Kotlin/Native in complex, multi-repository environments."
...
> Anton Arhipov ([slides](https://speakerdeck.com/antonarhipov/spec-driven-development-with-ai-agents-from-high-level-requirements-to-working-software-e397ad2e-3484-4bfa-a80b-6c47c593ed11), [video](https://www.youtube.com/watch?v=cTJorhnxrFI)). "AI coding agents are powerful, but they often feel unpredictable." The talk presents a structured spec-driven workflow to make agent output predictable.
...
- The SDD process: spec, criteria, rules, review, plan.
